...
const CheckoutCard: React.FC = () => {
	...
	// tax separation
	let total_tax_free=0.00;
	let total_tax=0.00;
	const { data} = useFlashSaleProductsQuery({
		limit: 10,
	});
	items.map((item:any)=>{
		data?.products.map((data:any)=>{
				if(item.id.substring(0,24)===data._id){
					total_tax_free = total_tax_free+ (parseFloat( data.tax_free_product_price)*parseFloat(item.quantity));
					total_tax= total_tax+(parseFloat( data.tax_price)*parseFloat(item.quantity));
				}
		})
	})
